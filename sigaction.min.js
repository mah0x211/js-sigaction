...
p);(new MutationObserver(q)).observe(document.body,{childList:!0,subtree:!0,attributes:!0});Array.prototype.slice.call(document.querySelectorAll("*[data-sigaction]")).forEach(m)}function l(a,b){if("string"!==typeof a)throw new TypeError("signame must be string");var d=g[a];if(d){var c=Array.prototype.slice.call(arguments),f=0;c.shift();d.forEach(function(e){try{e.g.apply(e.i,c),f++}catch(r){console.error(r)}});return f}return 0}var g={};window.addEventListener("DOMContentLoaded",p);window.sigaction=
{add:function(a,b,d){if("string"!==typeof a)throw new TypeError("signame must be string");if(!h(b))throw new TypeError("act must be function");var c=g[a];if(c){a=0;for(var f=c.length;a<f;a++)if(c[a].g===b)return!1}else c=[],g[a]=c;c.push({g:b,i:d||b});return!0},remove:function(a,b){if("string"!==typeof a)throw new TypeError("signame must be string");if(!h(b))throw new TypeError("act must be function");if(a=g[a])for(var d=0,c=a.length;d<c;d++)if(a[d].g===b)return a.splice(d,1),!0;return!1},raise:l};
h(window.SigactionLoaded)&&window.SigactionLoaded()})();
